...
const defaultOptions = (
	options: FormOptions = {}
): FormOptions & Required<Pick<FormOptions, "validateOn" | "reValidateOn" | "initialValues">> => ({
...
export const useForm = (options: FormOptions = {}) => {
	...
	const setValue = useCallback(
		(name: string) => (newValue: any, silent = false) => setValues({ [name]: newValue }, silent),
		[setValues]
	)
	const unsetValue = useCallback((name: string, silent = false) => unsetValues([name], silent), [unsetValues])
	...
	const validate = useCallback(async (fieldNames?: string[]) => {
		...
		const unsetFields = fieldNames.filter((fieldName) => !result.errors![fieldName])
	...
	const setState = useCallback((state: Partial<ReturnType<typeof getState>>, silent = false) => {
...
export const useFormContext = (formCtx?: FormCtx) => {
	...
	if (!ctx) {
		throw new Error("No FormContext found. Did you forget to wrap this component in a <FormProvider>?")
	}
	return ctx
}
