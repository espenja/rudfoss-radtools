 ...
 */
export const useWatchObject = <TPropertyType>(initialValues: WatcherObject<TPropertyType> = {}) => {
	/**
	...
	/**
	 * Returns a new object containing a subset of the properties in the live object.
	 */
	 ...
	 */
	const registerWatcher = useCallback((watcherId: string, updater: WatcherUpdater, propNames?: string | string[]) => {
		...
		for (const propName of arrayPropNames) {
			const watchersList = propNameWatcherIdsRegistry.current.get(propName) || new Set<string>()
	...
	const unregisterPropWatcher = useCallback((watcherId: string) => {
		...
		if (watchedPropNames) {
			...
			for (const propName of watchedPropNames) {
				const propNameWatchers = propNameWatcherIdsRegistry.current.get(propName)
				if (propNameWatchers) {
	...
	const notifyWatchers = useCallback((changedPropNames: string[]) => {
		...
		for (const changedPropName of changedPropNames) {
			const propWatchers = propNameWatcherIdsRegistry.current.get(changedPropName)
			if (propWatchers) {
		...
		for (const watcherIdToNotify of watcherIdsToNotify) {
			callWatcher(watcherIdToNotify, allPropsWatcherIdsRegistry.current.has(watcherIdToNotify))
		}
	}, [])
	const setValues = useCallback((newValues: WatcherObject<TPropertyType>, doNotNotify = false) => {
	...
	const unsetValues = useCallback((props?: string[], doNotNotify = false) => {
		...
		const unsettableProps = props.filter((prop) => valueObject.current[prop] !== undefined)
